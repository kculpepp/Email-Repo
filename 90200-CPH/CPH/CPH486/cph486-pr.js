...
//Custom Fields and Validations
    ...
    function initDropdown(obj, cb, flag) {
        ...
        if (flag) {
            ...
            $(obj).chosen().change(function () {$(this).removeClass('error'); resetHeight(); });
        }
...
//Local Storage
    ...
    function putParams() {
        ...
        if (url.parameters.eml) {$('[name=candidate_email]').val(url.parameters.eml); }
    }
...
//Scroll Events
    ...
    function scroll(s, p) {
        if (svars.compatible) {
            ...
            /*if (flg.form && wX > 991 && wY > 400) {
                if (s > 161 && (p < 97 || s < 2100)) {
                    var x = s - 161;
                    $('.form-container .padding').attr('style', 'margin-top:' + x + 'px;top:10px;position:relative');
                } else { $('.form-container .padding').attr('style', 'margin-top:0;top:0;margin-bottom:10px'); }
            } else {
                $('.form-container .padding').attr('style', 'margin-top:0;top:0;margin-bottom:10px');
            }*/
